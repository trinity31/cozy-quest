...
> 2026-05-22 갱신. 빡세게 + 옵션 없는 심플 룰 (GAME_RULES §M·§N 정합).

### 12.1 시간 제한 — 빡세게 (Day별 차등)

- **씬당 60~90초 Day별 차등** — Day 1 쉬움 ~90초, Day 5 어려움 ~60초 (어려울수록 짧음 = 도전 강화)
- **빡세게 적용**. 관대한 타이머(120~180초)는 "없는 거랑 동일"이므로 폐기
- **옵션 토글·난이도 선택·"투어 모드" 일체 없음** (심플 원칙)
- 중간값(Day 2·3·4)은 Phase 2 구현 시 결정
- **타이머 UI**: 상단 진행 바 + 잔여 초 숫자
- **남은 시간 10초**: 색상 변경 (붉은 톤) + SFX `time_warning` (P1)
- **시간 초과 처리** = **씬 재시작 버튼만 표시**
  - 재시작 횟수 제한 없음
  - 자동 다음 날 진행 X (복잡함, 흐름 끊김)
  - 사용자가 직접 재시작 누르고 클리어해야 다음 날로 진행
  - 미발견 부위 자동 노출·별 등급(stars) 폐기 (관대 룰 제거)
...
- **광고 시청 +1 시스템 없음** (인라인 placeholder·광고 자리 버튼 폐기)
- **광고 정책**: 정식 출시 = 전면 광고만 (씬 진입 전 또는 클리어 후). 해커톤 데모 = 광고 완전 OFF
...
// 게임 진행 상태 (localStorage)
{
  ...
  "current_day": 3,
  "scene_progress": {
    "market_dawn_001": { "completed": true, "time_used_seconds": 38, "hints_used": 0 },
...
- **v1.3 (2026-05-22) — §12 빡세게 모드 갱신** ⭐ (GAME_RULES §M·§N 정합, Obsidian §L 결정 반영)
  - §12.1 시간 표 방향 뒤집기 — Day 1 ~90초 / Day 5 ~60초 (어려울수록 짧음). 옵션 토글·관대 타이머 폐기
  - §12.1 시간 초과 = 씬 재시작 버튼만, 자동 다음 날 X. 별 등급(stars)·미발견 자동 노출 폐기
  - §12.2 광고 +1 시드 폐기 → 전면 광고만(정식 출시), 데모 OFF. 인라인 placeholder 제거
  - §12.4 localStorage 예시에서 `stars` 필드 제거
