...
- **Day 4 (목, 5/21)** ✅ — Sprint 정리: 박제 컷(v2 백로그), Day 5 범위 확정
- **Day 5 (금, 5/22)** ⏳ — 콘텐츠 파이프라인 ⚠️ 위험 (Day 2~7 풍경 6장 + 6 카테고리 × 3 variant = 18 furniture, 시즌 1 slots 좌표 카테고리별 동시 확정)
...
### Day 4 ✅ (Sprint 정리)
- [x] 24h 박제 v2 백로그로 컷 (1일 1풍경 구조상 발현 경로 없음, Day 6 시즌 갤러리 만들 때 재고려)
- [x] silhouette + Day N 잠금 힌트 컷 (시각적 잡음 vs 코지함 — Trinity 결정)
- [x] 6 카테고리 슬롯 좌표 정의는 Day 5 콘텐츠 파이프라인으로 흡수

### Day 5 ⏳ (콘텐츠 파이프라인)
- [ ] Day 2~7 풍경 6장 생성 + 좌표 추출 (`/scenes/` PNG, `scenes.json`)
- [ ] 6 카테고리 × 3 variant PNG (총 18장, `/furniture/season_001/`)
- [ ] `seasons.json` `slots`에 6 카테고리 좌표 추가
- [ ] `scenes.json`에 Day 2~7 씬 + 카테고리 매핑
...
- **24h 박제** — 어제 못 찾은 부위 회색 표시. 시즌 갤러리/도감(Day 6+ 또는 W22+)에서 어제 씬 진입 경로 생기면 재고려
- **슬롯 silhouette + Day N 잠금 힌트** — 보금자리 미발견 카테고리 표시. 시각적 잡음 우려로 컷, 시즌 갤러리 디자인 시 재고려
