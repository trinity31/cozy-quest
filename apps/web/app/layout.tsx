...
export default function RootLayout({
  ...
  return (
    <html lang="ko" className={`${gowun.variable} ${caveat.variable}`}>
      <body className="flex min-h-[100dvh] items-center justify-center bg-paper-deep text-text antialiased">
        {/* 9:16 모바일 프레임 — 모든 화면에서 9:16 박스 + 둥근 모서리 + ink 테두리 + 그림자
            (GAME_RULES §O.1, SEASON1 §9.1: max-width 420px + 코지 톤 배경 + 세로 모드 고정)
            width = 420 / 100vw / 96dvh*9/16 중 최소.
            dvh = dynamic viewport height — 모바일 Chrome URL 바 노출 시 vh가 가시 영역을
            초과해서 9:16 프레임이 상하 잘리는 문제 해결. */}
        <div
          className="mx-auto flex aspect-[9/16] flex-col overflow-hidden bg-paper-soft
                     w-[min(420px,100vw,calc(96dvh*9/16))]
